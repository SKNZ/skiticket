...
public class Reader {
    ...
    public static boolean readMemory(byte[] target, boolean auth, boolean display) {
        ...
        for (int i = 0; i < 44; i++) {
            ...
            try {
                ...
                disconnect();
                Toast.makeText(MyActivity.outer, "Reading ended on page " + i, Toast.LENGTH_SHORT).show();
    ...
    public static boolean testAuthenticate() {
        if (connect()) {
            ...
            if (result)
                Toast.makeText(MyActivity.outer, "Authentication succeeded", Toast.LENGTH_SHORT).show();
            else
    ...
    public static boolean authenticate(byte[] input_key, boolean display) {
        ...
        try {
            ...
            if (display) {
                //Toast.makeText(MyActivity.outer, "Authentication OK", Toast.LENGTH_SHORT).show();
                Reader.history += "\nAuthentication OK\n--------------------------------\n";
            }
            return true;
